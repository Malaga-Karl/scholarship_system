  ...
  const handleSend = async () => {
      const emailData = {
        message: {
          ...
          toRecipients: [
            {
              emailAddress: { address: student_email },
            },
          ],
          ccRecipients: cc
            ? [
      ...
      {loading? (
        ...
        <Box padding={5}>
          ...
          <Box mt={1} sx={{ display: "flex", justifyContent: "space-between" }}>
            <Button variant="contained" onClick={() => (navigate(`/adminView/applicants`))}>
                Back
            </Button>
            <Button variant="contained" color="success" onClick={() => (navigate(`/adminView/applicants/emails/${student_email}`))}>
                Emails
            </Button>
